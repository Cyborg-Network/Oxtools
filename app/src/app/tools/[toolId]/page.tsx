...
function ToolPageContent({ toolId }: { toolId: string }) {
  ...
  const [fields, setFields] = useState<Record<string, string>>(() => {
    ...
    for (const input of tool.inputs) {
      initial[input.key] = input.type === "select" && input.options?.length
  ...
  const isReady = tool.requiredFields.every(
      ...
      <div className="space-y-6">
          ...
          {toolUsage.limitReached ? (
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                ...
                <button onClick={redirectToUpgrade} className="text-primary hover:underline underline-offset-2">
        ...
        {!toolUsage.limitReached && toolUsage.remaining <= 2 && toolUsage.remaining > 0 && (
          <p className="text-xs text-amber-500">
            ⚡ {toolUsage.remaining} use{toolUsage.remaining === 1 ? "" : "s"} remaining for this tool today.{" "}
            <button onClick={redirectToUpgrade} className="underline underline-offset-2 hover:text-amber-400">
              Upgrade for more
            </button>
          </p>
        )}
