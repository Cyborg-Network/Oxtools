...
import { Lock, Sparkles, TerminalSquare } from "lucide-react";
	...
	return (
		<div className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
				{/* ── MODE A ── */}
				{/* biome-ignore lint/a11y/useSemanticElements: must be <div> to allow nested <button> inside */}
				<div
					role="button"
					tabIndex={0}
					onClick={() => setMode("A")}
					...
					{mode === "A" && (
						...
						>
							<div className="space-y-1">
								<label htmlFor="nl-query" className="text-xs font-semibold text-foreground/80">
									Describe your query
								</label>
								<textarea
									id="nl-query"
							...
							<div className="space-y-1">
								<label htmlFor="sql-dialect" className="text-xs font-semibold text-foreground/80">
									SQL Dialect
								</label>
								<select
									id="sql-dialect"
					...
					<div className="flex items-center gap-3 mb-3">
						...
						<div>
							...
							<p className="text-xs text-muted-foreground">
								Mode B ·{" "}
								{hasSuccessfulGeneration ? "Unlocked" : "Requires one successful generation"}
							</p>
						</div>
					</div>
					...
					{hasSuccessfulGeneration && mode === "B" && (
						...
						>
							...
							<div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl text-amber-700 dark:text-amber-400 text-xs flex gap-2 items-start">
								...
								<p>
									Runs against <strong>synthetic mock data</strong> generated from your schema. Your
									real database is never accessed.
								</p>
							</div>
							...
							<p className="text-[10px] text-muted-foreground">
								Dialect:{" "}
								<strong>
									{DIALECTS.find((d) => d.value === dialect)?.label ?? dialect.toUpperCase()}
								</strong>{" "}
								(same as Mode A)
							</p>
